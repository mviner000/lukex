...
const ShapesMenu = ({
    ...
    return (
        <>
            <DropdownMenu>
                <DropdownMenuTrigger asChild className="bg-transparent no-ring">
                ...
                <DropdownMenuContent className=" mt-5 flex flex-col gap-y-1 border-none bg-primary-black py-4 text-white">
                    {item.value.map((elem) => (
                        <Button
                            ...
                            className={`dark:bg-emerald-500 flex h-fit justify-between gap-10 rounded-none px-5 py-3 focus:border-none ${activeElement.value === elem?.value ? "bg-primary-green" : "hover:bg-primary-grey-200"
                                }`}
                        >
                            <div className="group flex items-center gap-2">
                                ...
                                />
                                <p
                                    className={`text-sm   ${activeElement.value === elem?.value ? "text-primary-black" : "dark:text-white"
                                        }`}
                                >
                                    {elem?.name}
                                </p>
